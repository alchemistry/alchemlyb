...
def generate_input_linear_approximation(parameter, parameter_range, parameter_change, pair_style, types_solvent, types_solute, 
                                        output_file=None, parameter2=None, parameter2_value=None, pair_style2=None):
    ...
    if nblocks % 1 > 0:
        raise ValueError("The number of steps needed to traverse the parameter range, {}, with a step size of, {} is not an integer".format(
            parameter_range, parameter_change))
    else:
        nblocks = int(nblocks)
        
    if (any([x is not None for x in [parameter2, pair_style2, parameter2_value]]) and
        not all([x is not None for x in [parameter2, pair_style2, parameter2_value]])):
        raise ValueError((f"If any values for 'parameter2' are provided, all must be provided: parameter2={parameter2}, "
...
def generate_traj_input(parameter, parameter_range, parameter_change, pair_style, types_solvent, types_solute, del_parameter=0.01, 
                        output_file=None, parameter2=None, parameter2_value=None, pair_style2=None, del_parameter2=None):
    ...
    nblocks = (parameter_range[1] - parameter_range[0]) / parameter_change
    if nblocks % 1 > 0:
        raise ValueError(f"The number of steps needed to traverse the parameter range, {parameter_range}, with a step size of, {parameter_change} is not an integer")
    else:
        nblocks = int(nblocks)
        
    if (any([x is not None for x in [parameter2, pair_style2, parameter2_value, del_parameter2]]) and
        not all([x is not None for x in [parameter2, pair_style2, parameter2_value, del_parameter2]])):
        raise ValueError((f"If any values for 'parameter2' are provided, all must be provided: parameter2={parameter2}, "
...
def generate_rerun_mbar(parameter_value, parameter, parameter_range, parameter_change, pair_style, types_solvent, 
                        types_solute, output_file=None, parameter2=None, pair_style2=None, parameter2_value=None):
    ...
    if np.isclose(parameter_range[0], 0):
        ...
        prec = max(int(np.abs(int(np.log10(np.abs(parameter_range[0]))))), int(np.abs(int(np.log10(np.abs(parameter_change)))+1)))
    name1 = "-".join([pair_style.replace("/","-"), parameter])
    file = [
        "\n# Variables and System Conditions\n",
    ...
    for i in range(nblocks):
        ...
        tmp = [
            ...
            "    c_FEP{0:03d}[1] c_FEP{0:03d}[2] c_FEP{0:03d}[3]".format(i)+f" file files/mbar_{name1}"+"_${param}_${param"+str(
                "{0:03d}".format(i))+"}.txt\n\n",
        ]
        if parameter2 is not None:
            ind = [ii for ii, x in enumerate(tmp) if "fix FEPout" in x][0]
            tmp[ind:ind+2] = [
                "fix FEPout{0:03d} all".format(i)+" ave/time ${freq} 1 ${freq} "+"v_time v_param v_param{0:03d} v_param2 &\n".format(i),
                "    c_FEP{0:03d}[1] c_FEP{0:03d}[2] c_FEP{0:03d}[3]".format(i)+f" file files/mbar_{name1}"+"_${param}_${param"+str(
                    "{0:03d}".format(i))+"}_"+"{}_{}.txt\n\n".format(name2, parameter2_value),
            ]
        file.extend(tmp)
        
    if parameter2 is not None:
        file.append(f"\nrerun files/dump_{name1}_"+"${param}_"+f"{name2}_{parameter2_value}.lammpstrj "
...
def _get_bar_lambdas(fep_files, indices=[2,3]):
    ...
    def tuple_from_filename(filename, separator="_", indices=[2,3]):
        ...
        if not _isfloat(name_array[indices[0]]):
            raise ValueError(f"Entry, {indices[0]} in filename cannot be converted to float: {name_array[indices[0]]}")
        if not _isfloat(name_array[indices[1]]):
            raise ValueError(f"Entry, {indices[1]} in filename cannot be converted to float: {name_array[indices[1]]}")
    ...
    def lambda2_from_filename(filename, separator="_", index=-1):
        ...
        if not _isfloat(name_array[index]):
            raise ValueError(f"Entry, {index} in filename cannot be converted to float: {name_array[index]}")
        return float(name_array[index])
    
    lambda_pairs = [tuple_from_filename(y, indices=indices) for y in fep_files]
    if len(indices) == 3:
        lambda2 = list(set([lambda2_from_filename(y, index=indices[2]) for y in fep_files]))
        if len(lambda2) > 1:
            raise ValueError("More than one value of lambda2 is present in the provided files."
    ...
    if check_float:
        raise ValueError("Lambda values must be convertible to floats: {}".format(check_float))
    ...
    for lambda_value, lambda_array in lamda_dict.items():
        missing_combinations_mbar.extend([(lambda_value, x) for x in lambda_values if x not in lambda_array])
        
    if missing_combinations_mbar:
        warnings.warn(
            "The following combinations of lambda and lambda prime are missing for MBAR analysis: {}".format(missing_combinations_mbar)
        )
    else:
    ...
    for ind, (lambda_value, lambda_array) in enumerate(lamda_dict.items()):
        if ind == 0:
            ...
            tmp_array = [lambda_values[ind-1], lambda_values[ind], lambda_values[ind+1]]

        missing_combinations_bar.extend([(lambda_value, x) for x in tmp_array if x not in lambda_array])
        extra_combinations_bar.extend([(lambda_value, x) for x in lambda_array if x not in tmp_array])
        
    if missing_combinations_bar:
        raise ValueError(
            "BAR calculation cannot be performed without the following lambda-lambda prime combinations: {}".format(missing_combinations_bar)
        )
    if extra_combinations_bar:
        warnings.warn(
            "The following combinations of lambda and lambda prime are extra and being discarded for BAR analysis: {}".format(extra_combinations_bar)
        )
...
@_init_attrs
def extract_u_nk(fep_files, T, columns_lambda1=[2,3], column_u_nk=4, column_lambda2=None, indices=[1,2], units="real", 
                 vdw_lambda=1):
    ...
    if units == "real":
        ...
        raise ValueError(f"LAMMPS unit type, {units}, is not supported. Supported types are: real and lj")

    if len(columns_lambda1) != 2:
        raise ValueError(f"Provided columns for lambda1 must have a length of two, columns_lambda1: {columns_lambda1}")
    ...
    for file in files:
        ...
        if [False for x in col_indices if x > lx]:
            raise ValueError("Number of columns, {}, is less than index: {}".format(lx, col_indices))
        ...
        if column_lambda2 is None:
            ...
            if vdw_lambda == 1:
                data.columns=["time", "vdw-lambda", "vdw-lambda2", "coul-lambda", "u_nk"]
                lambda1_col, lambda1_2_col = "vdw-lambda", "vdw-lambda2"
                columns_b = [(lambda2, x) for x in lambda_values]
            elif vdw_lambda == 2:
                data.columns=["time", "coul-lambda", "coul-lambda2", "vdw-lambda", "u_nk"]
        ...
        for lambda1 in list(data[lambda1_col].unique()):
            ...
            for lambda12 in list(tmp_df[lambda1_2_col].unique()):
                ...
                column_name = lambda_values[
                    [ii for ii,x in enumerate(lambda_values) if float(x) == lambda12][0]
                ]
                if column_lambda2 is not None:
                    column_name = (lambda2, column_name) if vdw_lambda == 1 else (column_name, lambda2)
                if u_nk.loc[u_nk[lambda1_col] == lambda1, column_name][0] != 0:
                    raise ValueError("Energy values already available for lambda, {}, lambda', {}.".format(lambda1, lambda12))
                
                if u_nk.loc[u_nk[lambda1_col] == lambda1, column_name].shape[0] != tmp_df2["u_nk"].shape[0]:
                    raise ValueError("Number of energy values in file, {}, N={}, inconsistent with previous files of length, {}.".format(
                        file,
                        tmp_df2["u_nk"].shape[0],
                        u_nk.loc[u_nk[lambda1_col] == lambda1, column_name].shape[0],
                    ))
                
                u_nk.loc[u_nk[lambda1_col] == lambda1, column_name] = beta * tmp_df2["u_nk"]
...
@_init_attrs
def extract_dHdl(fep_files, T, column_lambda1=2, column_dlambda1=3, column_lambda2=None, 
                 column_dlambda2=None, columns_derivative1=[10,11], columns_derivative2=[12,13], index=-1, units="real"):
    """ This function will go into alchemlyb.parsing.lammps
    ...
    if not isinstance(column_lambda1, int):
        raise ValueError("Provided column_lambda1 must be type 'int', instead: {}".format(type(column_lambda1)))
    if column_lambda2 is not None and not isinstance(column_lambda2, int):
        raise ValueError("Provided column_lambda2 must be type 'int', instead: {}".format(type(column_lambda2)))
    if not isinstance(column_dlambda1, int):
        raise ValueError("Provided column_dlambda1 must be type 'int', instead: {}".format(type(column_dlambda1)))
    if column_dlambda2 is not None and not isinstance(column_dlambda2, int):
        raise ValueError("Provided column_dlambda2 must be type 'int', instead: {}".format(type(column_dlambda2)))

    if len(columns_derivative1) != 2:
        raise ValueError("Provided columns for derivative values must have a length of two, columns_derivative1: {}".format(
            columns_derivative1))
    ...
    if len(columns_derivative2) != 2:
        raise ValueError("Provided columns for derivative values must have a length of two, columns_derivative2: {}".format(
            columns_derivative2))
    ...
    if column_lambda2 is None:
        ...
        dHdl = pd.DataFrame(columns=["time", "coul-lambda", "vdw-lambda", "coul", "vdw"])
        col_indices = ([0, column_lambda2, column_lambda1, column_dlambda1, column_dlambda2] 
                       + list(columns_derivative1) + list(columns_derivative2))
    ...
    for file in files:
        ...
        if column_lambda2 is None:
            ...
            data.columns = [
                "time", "coul-lambda", "vdw-lambda", "dlambda_vdw", "dlambda_coul", 
                "dU_back_vdw", "dU_forw_vdw", "dU_back_coul", "dU_forw_coul",
            ]          
            data["coul"] = ( data.dU_forw_coul - data.dU_back_coul ) / ( 2 * data.dlambda_coul )
            data["vdw"] = ( data.dU_forw_vdw - data.dU_back_vdw ) / ( 2 * data.dlambda_vdw )
            data.drop(columns=["dlambda_vdw", "dlambda_coul", "dU_back_coul", "dU_forw_coul",
                               "dU_back_vdw", "dU_forw_vdw"], inplace=True)  
