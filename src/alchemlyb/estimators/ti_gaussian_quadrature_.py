...
class TI_GQ(BaseEstimator, _EstimatorMixOut):
    ...
    def fit(self, dHdl):
        ...
        lambda_list, means_list, variances_list = self.separate_mean_variance(means, variances)
        for lambdas in lambda_list:
            num_lambdas = len(lambdas)
            if (num_lambdas not in self.special_points) or (not np.allclose(lambdas, self.special_points[num_lambdas]['lambdas'], rtol=0.1)):
    ...
    def separate_mean_variance(means, variances):
        ...
        if len(means.index.names) == 1:
            ...
            if (((0.0 < lambdas) & (lambdas < 1.0)).sum(axis=1) > 1.0).any():
                raise ValueError('Scaling multiple lambdas at the same time is not supported, please use trapezoidal rule instead.')
