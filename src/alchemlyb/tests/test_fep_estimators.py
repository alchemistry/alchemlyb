...
def gmx_benzene_coul_u_nk():
    ...
    u_nk = alchemlyb.concat(
        [gmx.extract_u_nk(filename, T=300) for filename in dataset["data"]["Coulomb"]]
    )
...
def gmx_benzene_vdw_u_nk():
    ...
    u_nk = alchemlyb.concat(
        [gmx.extract_u_nk(filename, T=300) for filename in dataset["data"]["VDW"]]
    )
...
class TestMBAR_fail:
    ...
    def n_uk_list(self):
        n_uk_list = [
            gmx.extract_u_nk(dhdl, T=300) for dhdl in load_ABFE()["data"]["complex"]
        ]
        return n_uk_list

    def test_failback_adaptive(self, n_uk_list):
        # The hybr will fail on this while adaptive will work
        mbar = AutoMBAR().fit(alchemlyb.concat([n_uk[:2] for n_uk in n_uk_list]))
        assert np.isclose(
