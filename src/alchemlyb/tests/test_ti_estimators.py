...
def gmx_benzene_coul_dHdl():
    ...
    dHdl = alchemlyb.concat(
        [gmx.extract_dHdl(filename, T=300) for filename in dataset["data"]["Coulomb"]]
    )
...
def gmx_benzene_vdw_dHdl():
    ...
    dHdl = alchemlyb.concat(
        [gmx.extract_dHdl(filename, T=300) for filename in dataset["data"]["VDW"]]
    )
...
def amber_simplesolvated_vdw_dHdl():
    ...
    dHdl = alchemlyb.concat(
        [amber.extract_dHdl(filename, T=298.0) for filename in dataset["data"]["vdw"]]
    )
...
class TestTI(TIestimatorMixin):
    ...
    @pytest.fixture(
        scope="class",
        params=[
            (gmx_benzene_coul_dHdl, 3.089, 0.02157),
...
class Test_MultipleColumnUnits:
    ...
    def dhdl():
        ...
        dhdl = alchemlyb.concat([extract_dHdl(data[i], 300) for i in range(30)])
        return dhdl
